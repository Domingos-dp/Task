  ...
  <div class="min-vh-100 bg-light p-3 p-sm-5 text-dark">
    <div class="container" style="max-width: 900px;">
      ...
      <header class="mb-5 text-center text-sm-start">
        <h1 class="display-4 fw-bold text-dark d-flex align-items-center justify-content-center justify-content-sm-start gap-2">
          <span>🕸️</span> 
          <span class="text-primary bg-gradient">
            Grafo de Tarefas
          </span>
        </h1>
        <p class="mt-2 text-muted lead">Gerencie dependências complexas entre suas atividades.</p>
      </header>
      ...
      <section>
        <div v-for="area in sortedAreas" :key="area" class="mb-5">
          <!-- Cabeçalho da Área -->
          <h2 class="h4 fw-bold text-secondary border-bottom pb-2 mb-4 d-flex align-items-center gap-2">
            <span class="badge bg-light text-primary border border-primary-subtle rounded-pill">#</span>
            {{ area }}
            <span class="small fw-normal text-muted ms-auto">{{ getTasksForArea(area).length }} tarefas</span>
          </h2>
              ...
              <div class="card-body p-4">
                  ...
                  <div class="d-flex align-items-center gap-3 flex-grow-1">
                    ...
                    <div class="d-flex flex-column">
                      ...
                      <div class="d-flex gap-2 mt-1">
                        <span v-if="isTaskBlocked(task)" class="badge bg-danger bg-opacity-10 text-danger border border-danger-subtle">
                          🔒 Bloqueado por dependências
                        </span>
                        <span v-else-if="task.status === 'completed'" class="badge bg-success bg-opacity-10 text-success border border-success-subtle">
                          ✅ Concluído
                        </span>
                        <span v-else class="badge bg-primary bg-opacity-10 text-primary border border-primary-subtle">
                          ⏳ Pendente
                        </span>
                      </div>
                    </div>
                ...
                <div class="mt-4 pt-3 border-top">
                  <div class="row g-3 align-items-start">
                    ...
                    <div class="col-12 col-md">
                      <h3 class="h6 fw-bold text-muted text-uppercase mb-2 d-flex align-items-center gap-1" style="font-size: 0.75rem; letter-spacing: 0.05em;">
                      ...
                      <div class="d-flex flex-wrap gap-2">
                        <span v-if="task.dependencies.length === 0" class="small text-muted fst-italic py-1">
      ...
      <!-- Estado Vazio -->
      <div v-if="tasks.length === 0" class="text-center text-muted mt-5 py-5 bg-white rounded border border-2 border-dashed">
        <div class="display-1 mb-3 opacity-50">📝</div>
        <h3 class="h5 fw-normal text-secondary">Nenhuma tarefa encontrada</h3>
        <p class="mt-2 text-muted mx-auto" style="max-width: 300px;">Comece adicionando tarefas acima e depois vincule dependências entre elas.</p>
      </div>
